...
class FDM(TransformerMixin, BaseEstimator):
    ...
    def choose_n_components(self, eigenvals):
        ...
        if isinstance(self.n_components, int):
            ...
            if self.step == 0:
                ...
                self.n_components_ = np.argmin(np.abs(eigenvals)**self.step >self.n_components*np.abs(eigenvals[0])**self.step)
        else:
    ...
    def fit(self,
        ...
        self.eigenvecs = deterministic_vector_sign_flip(eigenvecs[:, 1:self.n_components_ + 1].T).T
